  ...
  <div className="rounded-2xl border border-white/10 bg-[#111420] p-4">
    ...
    {tasks.length ? (
      <div className="mt-3 space-y-2">
        {tasks.map((task) => (
          <div key={task._id || task.id} className="rounded-lg border border-white/5 bg-[#0d1018] px-3 py-2">
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">Your real-time productivity snapshot.</p>
        </div>
        <Link
          to="/tasks"
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <KpiCard label="Total" value={overview.kpis.total} icon={FiCalendar} accent="bg-blue-500/15 text-blue-300" />
        <KpiCard label="Due today" value={overview.kpis.dueToday} icon={FiClock} accent="bg-amber-500/15 text-amber-300" />
        <KpiCard label="Overdue" value={overview.kpis.overdue} icon={FiAlertCircle} accent="bg-red-500/15 text-red-300" />
        <KpiCard label="Completed (7d)" value={overview.kpis.completed7d} icon={FiCheckCircle} accent="bg-emerald-500/15 text-emerald-300" />
        <KpiCard label="Completion rate" value={`${overview.kpis.completionRate}%`} icon={FiCheckCircle} accent="bg-violet-500/15 text-violet-300" />
      </div>
          ...
          {overview.lists.recentActivity.length ? (
            <div className="mt-3 space-y-2">
              {overview.lists.recentActivity.map((item, idx) => (
                <div key={`${item.task?._id || item.task?.id || idx}-${item.type}`} className="rounded-lg border border-white/5 bg-[#0d1018] px-3 py-2">
                  <p className="text-sm text-gray-100">{item.task?.title}</p>
                  <p className="mt-1 text-xs capitalize text-gray-500">{item.type} · {formatDate(item.date)}</p>
                </div>
              ))}
            </div>
          ) : (
      ...
      <div className="rounded-2xl border border-white/10 bg-[#111420] p-4">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-white">Completion Trend (7 days)</h3>
          <span className="text-xs uppercase tracking-wide text-gray-500">Weekly</span>
        </div>
            ...
            return (
              <div key={item.date} className="flex flex-col items-center gap-1">
                <div className="flex h-24 w-full items-end rounded-md bg-[#0d1018] p-1">
                  <div className="w-full rounded-sm bg-blue-500/80" style={{ height: `${heightPercent}%` }} />
                </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#111420] p-4">
        ...
        <p className="text-sm text-gray-300">{insights?.summary || 'No insights available.'}</p>
        <div className="mt-3 space-y-2">
          {(insights?.suggestions || []).map((item) => (
            <div key={item.id} className="rounded-lg border border-white/5 bg-[#0d1018] p-3">
              ...
              <Link to={item.actionRoute || '/tasks'} className="mt-2 inline-block text-xs font-medium text-blue-400 hover:text-blue-300">
